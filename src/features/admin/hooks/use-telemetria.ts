...
import { telemetriaService } from 'src/features/admin/services/telemetria.service';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [logsData, alertasData] = await Promise.all([
        telemetriaService.getLogs(),
        telemetriaService.getAlertas(),
      ]);
      setLogs(logsData);
