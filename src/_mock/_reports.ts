...
export const mockInventoryRisk = () => {
  ...
  const low = riskProducts.filter(
    (p) => p.available > p.minStock && p.available <= p.minStock * 2
  ).length;
  const totalRisk = riskProducts.length;

  return {
    kpis: {
      'Agotados (Riesgo)': outOfStock,
      'Estado Crítico': critical,
      'Stock Bajo': low,
      'Total en Riesgo': totalRisk,
    },
    chartData: {
    ...
    tableData: riskProducts.map((p) => {
      ...
      const statusBadge =
        diff < 0 ? { label: 'Crítico', color: 'error' } : { label: 'Bajo', color: 'warning' };
      return {
        id: p.id,
        producto: p.name,
        sku: p.sku,
...
export const mockInventoryCategories = () => {
  const categories = MOCK_CATEGORIES.map((cat) => {
    ...
    return {
      ...
      atRisk: prods.filter((p) => p.stockStatus === 'out_of_stock' || p.stockStatus === 'low_stock')
        .length,
    };
...
export const mockInventoryB2B = () => {
  ...
  const items = activeQuotes.flatMap((q) =>
    q.items.map((i) => ({
      ...i,
      quoteId: q.id,
      client: q.clientName,
      ...
      series: [
        {
          ...
          data: clients
            .slice(0, 10)
            .map((c) => items.filter((i) => i.client === c).reduce((s, i) => s + i.quantity, 0)),
        },
      ],
    },
...
export const mockSalesProducts = () => {
  ...
  const tableData = richProducts
    ...
    .filter((x) => x.vecesCotizado > 0);

  return {
    kpis: {
      'Producto Estrella': tableData[0]?.producto || 'Ninguno',
      'Variedad (SKUs)': Object.keys(freqMap).length,
      'Promedio Cotizado': tableData.length
        ? Math.round(tableData.reduce((s, x) => s + x.unidades, 0) / tableData.length)
        : 0,
    ...
    chartData: {
      ...
      series: [
        {
          name: 'Frecuencia en cotizaciones',
          data: tableData.slice(0, 10).map((x) => x.vecesCotizado),
        },
      ],
    },
    tableData,
...
export const mockSalesDistributors = () => {
  ...
  return {
    kpis: {
      ...
      'Cotizaciones x Cliente': clients.length
        ? Math.round(MOCK_QUOTES.length / clients.length)
        : 0,
